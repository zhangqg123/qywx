...
//						    mainList.add(mainBtn);
			 		...
			 		AccessToken accessToken = accountService.getAccessToken(query.getAgentId());
					...
					System.out.println("...result..."+result+"....receive..."+accessToken);
			 		if(result == 0){
			 			j.setMsg("同步菜单成功!");
			 		}else{
					    log.warn("返回代码:",result);			 			
			 			j.setMsg("同步菜单失败!result="+result+";accessToken="+accessToken);
			 		}
			 	}
			} catch (Exception e) {
