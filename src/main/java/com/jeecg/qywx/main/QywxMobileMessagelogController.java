 ...
 */
@Controller
@RequestMapping("/qywx/qywxMobileMessagelog")
public class QywxMobileMessagelogController extends BaseController{
  @Autowired
  ...
  private QywxGroupDao qywxGroupDao;
	...
	@RequestMapping(params = "mobileList",method = {RequestMethod.GET,RequestMethod.POST})
	public void mobileList(@ModelAttribute QywxMessagelog query,HttpServletRequest request,HttpServletResponse response,
			@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
			@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
		try {
			LOG.info(request, " front list");
//			String fromUserId=(String) request.getSession().getAttribute("QY_USERID");
			String fromUserId="zhangqg";
			List<QywxMessagelog> sendList = qywxMessagelogDao.getSendLogs(fromUserId);
			List<QywxMessagelog> receiveList = qywxMessagelogDao.getReceiveLogs(fromUserId);
			VelocityContext velocityContext = new VelocityContext();
//			for (QywxMessagelog messagelogPo : list) {
...
//			velocityContext.put("qywxMessagelog", query);
			velocityContext.put("pageInfos", sendList);
			velocityContext.put("pageInfos2", receiveList);
			String viewName = "qywx/main/qywxMobileMessage.vm";
