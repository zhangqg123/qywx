		...
		<#if qywxGzuserinfo.department ?exists && qywxGzuserinfo.department ?length gt 0>
		    /* 部门 select * from qywx_gzuserinfo where concat(',',department,',') regexp concat(',(',replace('71768,80129',',','|'),'),');*/
			/* and qg.department in (select id from qywx_group WHERE parentid = :qywxGzuserinfo.department order by id)*/
