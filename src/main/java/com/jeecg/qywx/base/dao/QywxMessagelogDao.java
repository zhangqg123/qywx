...
public interface QywxMessagelogDao{
	...
	@Sql("SELECT * FROM qywx_messagelog WHERE fromuser_id = :fromUserId")
	List<QywxMessagelog> getSendLogs(@Param("fromUserId") String fromUserId);

	@Sql("SELECT * FROM qywx_messagelog WHERE fromuser_id != :fromUserId")
	List<QywxMessagelog> getReceiveLogs(@Param("fromUserId") String fromUserId);
