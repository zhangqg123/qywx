...
/**
 * 
 * @author yangqj 描述：微信成员管理service实现类
 *
 */
@Service
public class QywxGzuserinfoServiceImpl implements QywxGzuserinfoService {
	...
	public void saveGzuserinfo(QywxGzuserinfo qywxGzuserinfo,
		...
		if (!StringUtils.isBlank(qywxGzuserinfo.getDepartment())) {
			user.setDepartment(new Integer[] { Integer.parseInt(qywxGzuserinfo
					.getDepartment()) });
		}
		...
		if (null != uploadfile && !uploadfile.isEmpty()) {
			try {
				...
				String current_dir = request.getServletContext().getRealPath(
						"/upload");// 当前路径
				String extension_name = uploadfile.getOriginalFilename()
						.substring(
				...
				// WXUpload.upload(accessToken.getAccesstoken(), "image",
				// current_dir+"//"+filename);
				// String url_temp =
				// "http://127.0.0.1:8080/jeecg-p3-web/upload/"+filename;
				// String url_temp =
				// "http://127.0.0.1:8080/jeecg-p3-web/upload/"+filename;
				String url_temp = "http://" + request.getServerName() + ":"
						+ request.getServerPort() + "/jeecg-p3-web"
						+ "/upload/" + filename;
				JSONObject jsonobject = WXUpload.upload(
						accessToken.getAccesstoken(), "image", url_temp);
				...
				// User temp_user = JwUserAPI.getUserByUserid(user.getUserid(),
	...
	@Override
	public void updateGzuserinfo(QywxGzuserinfo qywxGzuserinfo,
			MultipartFile uploadfile, HttpServletRequest request) {
		// 把值赋值给用户
		User user = new User();
		user.setUserid(qywxGzuserinfo.getUserid());
...
//			user.setDepartment(new Integer[] { Integer.parseInt(qywxGzuserinfo
//					.getDepartment()) });
			Integer[] departments = stringtoInteger(qywxGzuserinfo.getDepartment());
			user.setDepartment(stringtoInteger(qywxGzuserinfo.getDepartment()));
		}
		user.setPosition(qywxGzuserinfo.getPosition());
		user.setMobile(qywxGzuserinfo.getMobile());
		// user.setGender((Integer.parseInt(qywxGzuserinfo.getGender())+1)+"");
		...
		if (null != uploadfile && !uploadfile.isEmpty()) {
			try {
				...
				// String current_dir=;//当前路径
				// String
				// current_dir=request.getServletContext().getRealPath("/upload");//当前路径
				...
				// WXUpload.upload(accessToken.getAccesstoken(), "image",
				// current_dir+"//"+filename);
				// String url_temp = request.getRequestURI()
				// "http://127.0.0.1:8080/jeecg-p3-web/upload/"+filename;
				String url_temp = "http://" + request.getServerName() + ":"
						+ request.getServerPort() + "/jeecg-p3-web"
						+ "/upload/" + filename;
	...
	public void deleteGzuserinfo(QywxGzuserinfo qywxGzuserinfo) {
		AccessToken accessToken = accountService.getAccessToken();
		// 先获取bena的信息，然后进行删除
		QywxGzuserinfo temp_qywxGzuserinfo = qywxGzuserinfoDao
				.get(qywxGzuserinfo.getId());
		int result = JwUserAPI.deleteUser(temp_qywxGzuserinfo.getUserid(),
				accessToken.getAccesstoken());
		qywxGzuserinfoDao.delete(qywxGzuserinfo);
		// 删除失败我们也不用管吧
		/*
		 * if(result!=0){ //不知道这个事物怎么控制哦 throw new
		 * RuntimeException("本地成员删除成功，微信删除成员失败，错误代码="+result); }
		 */
	}
	...
	public void syncGzuserinfos() {
		...
		for (QywxGroup qywxGroup : qywxGroups) {
			// List<User> users =
			// JwUserAPI.getUsersByDepartid(qywxGroup.getId(), null, null,
			// accessToken.getAccesstoken());
			List<User> users = JwUserAPI
					.getDetailUsersByDepartid(qywxGroup.getId(), null, null,
							accessToken.getAccesstoken());
...
//				JSONObject extattr = user.getExtattr();
//				String attrs;
//				if (extattr!=null&&!StringUtils.isBlank(extattr.getString("attrs"))) {
//					attrs = extattr.getString("attrs");
//					String mm = attrs;
//				}
				QywxGzuserinfo temp_userinfo = qywxGzuserinfoDao
						.getByUserid(userid);
				if (null == temp_userinfo) {
					...
					if (null != user.getDepartment()) {
						...
						for(int i=0;i<user.getDepartment().length;i++){
							sb.append(user.getDepartment()[i] + ",");
						}
						save_user.setDepartment(sb.toString().substring(0,sb.length()-1));
					}
	...
	@Override
	public Map<String, String> getGzuserinfo(String userid) {
		...
		if (user != null) {
			...
			depId=user.getDepartment();
			if(depId.split(",").length>1){
				for (int i=0;i<depId.split(",").length;i++){
					str = str + qywxGroupDao.get(depId.split(",")[i]).getName()+",";
				}
				department=str.substring(0,str.length()-1); 
			}else{
			...
			map.put("quanxian", user.getQuanxian());
		} else {
		}
		return map;
	}
